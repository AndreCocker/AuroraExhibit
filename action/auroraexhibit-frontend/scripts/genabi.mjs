...
const HARDHAT_DEPLOYMENTS_ROOT = path.resolve(ROOT, "auroraexhibit-hardhat", "deployments");
...
function readDeployment(network) {
  ...
  const file = path.resolve(dir, "AuroraExhibit.json");
...
function main() {
  ...
  if (!localhost && !sepolia) {
    console.log("AuroraExhibit not deployed on localhost or sepolia. Skip genabi.");
    return;
  }

  // write ABI - prefer localhost ABI if exists, else sepolia
  const gg = localhost || sepolia;
  const abiTs = `export const AuroraExhibitABI = {\n  abi: ${JSON.stringify(gg.abi, null, 2)}\n};\n`;
  fs.writeFileSync(path.resolve(FRONTEND_ABI_DIR, "AuroraExhibitABI.ts"), abiTs);
  ...
  const addrTs = `export const AuroraExhibitAddresses = ${JSON.stringify(addresses, null, 2)} as const;\n`;
  fs.writeFileSync(path.resolve(FRONTEND_ABI_DIR, "AuroraExhibitAddresses.ts"), addrTs);
