...
export function useGallery(parameters: {
  ...
  const contractAddress = useMemo(() => {
    ...
    const entry = (AuroraExhibitAddresses as any)[chainId?.toString()];
  ...
  const refreshArtworks = useCallback(async () => {
    ...
    try {
      ...
      const contract = new ethers.Contract(contractAddress, AuroraExhibitABI.abi, rp);
      const ids: bigint[] = await contract.listPieces();
      const rows: ArtworkItem[] = [];
      for (const idb of ids) {
        const id = Number(idb);
        const [rid, artist, title, descriptionHash, fileHash, tags, categories, timestamp, likesHandle] = await contract.fetchPiece(id);
  ...
  const like = useCallback(async (id: number) => {
    ...
    try {
      ...
      const contract = new ethers.Contract(contractAddress, AuroraExhibitABI.abi, s);
      const tx = await contract.applaud(id);
      await tx.wait();
  ...
  const vote = useCallback(async (id: number, category: string) => {
    ...
    try {
      ...
      const s = await bp.getSigner();
      const contract = new ethers.Contract(contractAddress, AuroraExhibitABI.abi, s);
      const tx = await contract.endorse(id, category);
      await tx.wait();
  ...
  const mockUpload = useCallback(async () => {
    ...
    try {
      ...
      const tx = await contract.mintPiece("Untitled", "ipfs://descHash", "ipfs://fileHash", tags, categories);
      await tx.wait();
      await refreshArtworks();
    } catch (e: any) {
